...
>
> **Compatibility with global gradient norm**: MultiMax scalar parameters are shared across the full vocabulary, so their gradients accumulate over all vocabulary entries and their gradient magnitude scales as `~sqrt(V)`. When they share a single global gradient norm with the backbone (`ClipGradByGlobalNorm` / `clip_grad_norm_`), this large term inflates that norm and shrinks the backbone's updates. Put MultiMax parameters in their own gradient-norm group, so their norm is computed separately and never affects the backbone's.
