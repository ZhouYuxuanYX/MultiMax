...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            ## retrieve ranges and ts for recmax
            m = layer.self_attn.ranges.cpu()
            t = layer.self_attn.ts.cpu()
            # g = b.attn.gates.cpu()
            # a = b.attn.acts.cpu()
            ranges.append(m)
            ts.append(t)
        ...
        # retrieve ranges and ts
        np.savez(f'stats/ranges.npz', *ranges)
        np.savez(f'stats/ts.npz', *ts)
        #exit()
        ...
        if self.project_out_dim is not None:
            print("project out dim")
            x = self.project_out_dim(x)
            #x = SeLU(x, self.ts, self.ranges)
    ...
    def output_layer(self, features):
        ...
        if self.adaptive_softmax is None:
            ...
            # verified, yes, adaptive softmax stuff is False
            print("-------use SeLU in the output------")
            return SeLU(self.output_projection(features), self.ranges, self.ts)
